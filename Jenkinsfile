                    ...
                    // Loop through each server and deploy the Docker container
                        ...
                        sh '''
                            ...
                            server='192.168.3.156'
                                ...
                                ssh -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa admin01@$server "sudo docker pull ${IMAGE_NAME}:${TAG}"
                                ssh -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa admin01@$server "sudo docker stop ${DOCKER_NAME} || true"
                                ssh -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa admin01@$server "sudo docker rm ${DOCKER_NAME} || true"
                                ssh -o StrictHostKeyChecking=no -i ~/.ssh/id_rsa admin01@$server "sudo docker run -d --name ${DOCKER_NAME} -p 8900:80 ${IMAGE_NAME}:${TAG}"
                        '''
